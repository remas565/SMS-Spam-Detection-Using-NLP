# -*- coding: utf-8 -*-
"""SMS_Spam_Detection_DistilBERT.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1wg2LodKcKsw8zltgCRgg2-UiB-Fa-5Zr
"""
...
def run_optimizer_experiment(opt_name, extra_kwargs={}):
    ...
    # Freeze DistilBERT layers (train classifier only)
    for param in model_exp.distilbert.parameters():
        param.requires_grad = False

    # Fixed learning rate (same as base model)
    lr_fixed = 0.0005
    ...
    # Optimizer comparison (only difference)
    if opt_name == "AdamW":
        from torch.optim import AdamW
        optimizer = AdamW(model_exp.parameters(), lr=lr_fixed)

    elif opt_name == "RMSProp":
        from torch.optim import RMSprop
        optimizer = RMSprop(model_exp.parameters(), lr=lr_fixed)
...
def run_lr_experiment(lr_value):
    ...
    # AdamW optimizer (fixed optimizer, varying LR)
    from torch.optim import AdamW
...
lr_results["LR_3e-5"] = run_lr_experiment(3e-5)

lr_results["LR_5e-4"]  = run_lr_experiment(5e-4)

lr_results["LR_1e-3"]  = run_lr_experiment(1e-3)
